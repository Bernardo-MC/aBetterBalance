...
function [bestFit,bestInd,BestFitIter,BestIndIter,FAccessIter] = MFO(FName,iterations,dims,bounds)
...
Moth_pos=initialization(N,dims,ub,lb);

BestIndIter=zeros(Max_iteration,dims);
BestFitIter=ones(Max_iteration,1)*Inf;
FAccessIter=zeros(Max_iteration,1);
FAccess=0;
...
while Iteration<Max_iteration+1
    ...
    for i=1:size(Moth_pos,1)
        ...
        Moth_fitness(1,i)=GetFitness(FName,Moth_pos(i,:));  
        FAccess = FAccess + 1;
    ...
    BestFitIter(Iteration)=Best_flame_score;
    BestIndIter(Iteration,:)=Best_flame_pos;
    FAccessIter(Iteration) = FAccess;
    
    %% Rate
    for r=1:N
        PopulationHistoric{Iteration,r} = Moth_pos(r,:);
    end
    %% Rate
...
bestFit=Best_flame_score;
bestInd=Best_flame_pos;

% %% Rate
% if exist('PopMFO.mat','file') == 2
%     load('PopMFO.mat');
