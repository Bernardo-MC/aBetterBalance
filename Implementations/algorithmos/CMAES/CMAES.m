...
function [bestFit,bestInd,BestFitIter,BestIndIter,FAccessIter] = cmaes(FName,iterations,dims,bounds)
%% Problem Settings

nVar=dims;                % Number of Unknown (Decision) Variables

VarSize=[1 nVar];       % Decision Variables Matrix Size

VarMin= bounds(1,1);             % Lower Bound of Decision Variables
VarMax= bounds(1,2);            % Upper Bound of Decision Variables
...
M(1).Cost=GetFitness(FName,M(1).Position);
...
for g=1:MaxIt
    ...
    M(g+1).Cost=GetFitness(FName,M(g+1).Position);
    ...
    %% Rate
    for l=1:lambda
        PopulationHistoric{g,l} = pop(l).Position;
    end
    %% Rate
    
    BestFitIter(g) = BestSol.Cost;
    BestIndIter(g,:) = BestSol.Position;
    FAccessIter(g) = FAccess;
    if(g==iterations-2)
       fprintf(''); 
    end
